...
function getConfig(): SiteConfig {
  // 这里应该从主站点的配置中获取
  return {
    address: process.env.NEXT_PUBLIC_MAIN_USDT_ADDRESS || '',
    qrcode: process.env.NEXT_PUBLIC_MAIN_USDT_QRCODE || '',
    customerService: {
      url: process.env.NEXT_PUBLIC_MAIN_CUSTOMER_SERVICE_URL || '',
      id: process.env.NEXT_PUBLIC_MAIN_CUSTOMER_SERVICE_ID || ''
    }
  };
}
...
export default function UsdtPayment() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!data.data?.customerService?.id || !data.data?.address || !data.data?.qrcode) {
  ...
  const handleTransferConfirm = async () => {
    ...
    try {
      ...
      if (data.success) {
        // 跳转到等待确认页面
        window.location.href = `/main/payment/waiting?orderNumber=${orderNumber}`;
      } else {
        throw new Error(data.message || '创建订单失败');
      }
    } catch (error: any) {
      console.error('Error creating transaction:', error);
      alert(error.message || '系统错误，请联系客服处理！');
