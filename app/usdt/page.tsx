...
export default function UsdtHome() {
  ...
  const handleAmountSelect = (amount: number, rate: number) => {
    ...
    window.location.href = `/payment/usdt?amount=${amount}&account=${accounts[key]}&cashback=${cashback}&orderId=${orderId}`;
  };
  ...
  const handleCustomAmountSubmit = () => {
    ...
    const orderId = `${Date.now()}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
    window.location.href = `/payment/usdt?amount=${amount}&account=${customAccount}&cashback=${cashback}&orderId=${orderId}`;
  };
